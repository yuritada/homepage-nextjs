...
summary: "予算3,000円で学部の年次発表会を作り直したプロジェクトの記録、第1回。プロジェクトの原点、現行フローの棚卸し、運営事務担当者へのヒアリング、そしてそこから引き出した機能一覧 F-01〜F-12 と非機能要件までをまとめます。"
series: "mirais"
seriesOrder: 1
...
先に全体像だけ掴みたい方は、[要約版の設計ノート](/blog/2026-09-02-mirais)からどうぞ。
...
開発費は **3,000 円**でした。この制約が、以下のアーキテクチャ判断を直接規定しています。
...
制約は設計の敵ではなく、判断を強制的に明確にしてくれる材料でした。「予算がないから諦める」ではなく「予算がないからこの構成になる」という順序で考えられたことが、結果的にアーキテクチャを筋の通ったものにしたと思っています。
...
結果として、3 つの検証はいずれも「動く」という結論を得て、そのまま本実装に移行しています。特に LLM については、検証の過程で「学内サーバーが使えない場合でも動く代替経路」を用意する判断につながりました（[第 32 章 / 第 5 回](/blog/2026-09-01-mirais-05-algorithms)）。
...
→ **2 フェーズ表彰アルゴリズム**（Phase 1: 教員推薦プールの生成 → Phase 2: 企業が序列順に推薦プールから選出し「学科賞 → 企業賞」に格上げ）が生まれました。[第 33 章 / 第 5 回](/blog/2026-09-01-mirais-05-algorithms)に詳述します。
...
実装状況は[第 42 章 / 第 6 回](/blog/2026-09-01-mirais-06-outcome)にまとめています。
