...
[前回](/blog/2026-07-16-mirais-01-origin)で定めた要件を、実際の構造へ落とし込む回です。6 ロールを前提としたサイトマップ、未ログインへの情報遮断ポリシー、技術スタックの選定理由、そして 20 テーブルの星型スキーマまでを扱います。
...
学内 LLM サーバーが設定されていればそれを使い、未設定または応答しない場合は**決定論的なモック埋め込み**にフォールバックします。**LLM が使えない環境でも推薦機能が停止しない**ことが設計の要点です（[第 32 章 / 第 5 回](/blog/2026-07-28-mirais-05-algorithms)）。
